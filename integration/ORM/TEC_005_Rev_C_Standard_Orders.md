...
## Standard Orders (ORM-O01) Inbound Specifications

Interface content encoding from EMR to IRIS v3.x using the HL7 ORM-O01 standard orders message.
...
This document outlines the values sent in an HL7 order message (ORM-O01) from third-party systems to the IRIS platform. This third-party system could be another application, an interface engine, or an EMR. For the purposes of this documentation, this third-party will be referred to an EMR. 

The following sections contain details of the ORM message content by HL7 Segment.  
...
| MSH-4  | Sending Facility (HD)  | Your facility name, e.g. "ANCL"  | Populated by EMR; This field can also be used to identify parent entities for the case of the sending body serving as a hub for multiple facilities. See Note 1.0 below  |
...
|   | PV1-3.1  | POC01  | Required. ID or abbreviation of the clinic.  |
...
| ORC-9  | Date/Time of Transaction (TS)  | yyyyMMddHHmmss format  | Required. Generated by EMR as order scheduled time  |
...
|   | …..  |   | ORC-13.5 through ORC-13.7 not necessary  |
...
| ORC-24  | Ordering Provider Address (XAD)  | ss  | Not necessary |
| -- | -- | -- | -- |
...
| OBR-43  | Planned Patient Transport Comment (CE)  | rt   | Not necessary  |
| OBR-44  | Procedure Code (CE)  |   | Not necessary  |
| OBR-45  | Procedure Code Modified | r (CE)  | Not necessary  |
...
| Field  | Name and type  | Sample value  | Description  |
| -- | -- | -- | -- |
| DG1-1  | Set ID - DG1 (SI)  | 1  | Required if sent. Increased sequentially by 1.  |
